...
import { Flag, Filter, ArrowUpRight, Calendar } from 'lucide-react'
import EmptyState from '../../../components/ai-forum/common/EmptyState.jsx'

/**
 * 举报处理页 - Phase0 占位骨架
 * 为什么只做占位：用户举报队列处理属于 Phase4 职责，当前阶段仅需页面骨架
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-af-lg p-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex-1 flex flex-wrap gap-2">
            {['处理状态', '举报类型', '被举报对象', '举报时间'].map((label) => (
              <div
                key={label}
      ...
      <div className="bg-card border border-border rounded-af-lg overflow-hidden">
        <EmptyState
          icon={Flag}
          title="Phase4 上线"
          description="用户举报队列处理功能 Phase4 交付，敬请期待"
          action={
            <Link
              to="/forum/admin"
